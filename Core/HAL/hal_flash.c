...
static int32_t stm32l5xx_flash_0_init_map_to_hal_flash_chip_0(void)
{
    HAL_FLASH_Unlock();
}
...
int32_t hal_flash_init(HAL_FLASH_CHIP *flash_ctx)
{
    ...
    switch (flash_ctx->num) {
        case HAL_FLASH_CHIP_0:
            ret = stm32l5xx_flash_0_init_map_to_hal_flash_chip_0();
            if (ret != HAL_SUCCESS) {
                goto finish;
            }
            break;
        case HAL_FLASH_CHIP_1:
            ...
            break;
#if LOCK_CODE
        case HAL_FLASH_CHIP_2:
        case HAL_FLASH_CHIP_3:
        case HAL_FLASH_CHIP_4:
#endif /* LOCK_CODE */
        default:
            ret = HAL_ERR_HW_LACK;
            break;
